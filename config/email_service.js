...
let isEmailConfigured = validateEmailConfig();
...
if (isEmailConfigured) {
    ...
    transporter.verify()
        ...
        .catch((error) => {
            ...
            logger.error('Emails will fail to send. Check EMAIL_USER and EMAIL_PASS credentials.');
...
const isEmailServiceReady = () => isEmailConfigured && transporter !== null;

// Retry logic for sending emails
const sendEmailWithRetry = async (mailOptions, maxRetries = 3) => {
    if (!isEmailServiceReady()) {
        throw new Error('Email service is not configured or initialized');
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            lastError = error;
            logger.warn(`Email send attempt ${attempt}/${maxRetries} failed: ${error.message}`);
...
const sendGroupInvitationEmail = async (to, inviterName, groupName, frontendUrl) => {
    ...
    try {
        const result = await sendEmailWithRetry(mailOptions);
        logger.info(`Group Invitation email sent to: ${to} (Message ID: ${result.messageId})`);
        return result;
    } catch (error) {
...
const sendPilgrimInvitationEmail = async (to, inviterName, groupName, deepLink) => {
    ...
    try {
        const result = await sendEmailWithRetry(mailOptions);
        logger.info(`Pilgrim invitation email sent to ${to} (Message ID: ${result.messageId})`);
        return result;
    } catch (error) {
